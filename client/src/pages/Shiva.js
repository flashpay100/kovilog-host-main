...
const Shiva = ({ addLike1, addLike, auth }) => {
  ...
  return isLoading ? (
    ...
    <div>
      ...
      <div className="Temples">
        <h1 className="Temples-heading">Shiva Temples</h1>
        <input
          className="Searchbar"
          placeholder="Search....."
          onChange={(event) => {
        ...
        {temples
          .filter((temple) => temple.deity === "Shiva")
          .sort((a, b) => {
            return a.temple_number - b.temple_number;
          })
          .filter((temple) => {
            if (
              temple.temple_name
...
export default connect(mapStateToProps, { addLike, addLike1, loadUser })(Shiva);
