...
const Nepal = ({ addLike1, addLike, auth }) => {
  ...
  return isLoading ? (
    ...
    <div>
      ...
      <div className="Temples">
        <h1 className="Temples-heading">Temples of Nepal</h1>
        <input
          className="Searchbar"
          placeholder="Search....."
          onChange={(event) => {
        ...
        {temples
          .filter((temple) => temple.location.includes("Nepal"))
          .filter((temple) => {
            if (
              temple.temple_name
                .toLowerCase()
...
export default connect(mapStateToProps, { addLike, addLike1, loadUser })(Nepal);
